...
// startWebServer starts a web server that listens on http://localhost:80.
...
func BuildOAuthHTTPClient(scope string, cfg *Config) (*http.Client, error) {
        config, err := readConfig(scope, cfg)
        if err != nil {
